package ing.boykiss.gmtk25.actor.interactable;

import com.badlogic.gdx.graphics.g2d.Animation;
import com.badlogic.gdx.graphics.g2d.Batch;
import com.badlogic.gdx.graphics.g2d.TextureRegion;
import com.badlogic.gdx.math.Vector2;
import com.badlogic.gdx.physics.box2d.*;
import ing.boykiss.gmtk25.Constants;
import ing.boykiss.gmtk25.registry.AssetRegistry;
import ing.boykiss.gmtk25.utils.AnimationUtils;
import lombok.Getter;

public class InteractableSwitch extends Interactable {

    @Getter
    private final Body body;

    @Getter
    private final Vector2 position;

    @Getter
    private final Runnable callback;

    private final Animation<TextureRegion> textures = AnimationUtils.createAnimationSheet(AssetRegistry.SWITCH_TEXTURE, 2, 1, new int[]{0, 1}, 0.2f);

    private final IInteractionTarget interactionTarget;

    private boolean isActive = false;

    public InteractableSwitch(World world, Vector2 position, IInteractionTarget interactionTarget) {
        BodyDef bodyDef = new BodyDef();
        bodyDef.type = BodyDef.BodyType.StaticBody;

        body = world.createBody(bodyDef);
        body.setUserData(this);

        PolygonShape shape = new PolygonShape();
        shape.setAsBox(6.0f * Constants.UNIT_SCALE, 4.0f * Constants.UNIT_SCALE);

        FixtureDef fixtureDef = new FixtureDef();
        fixtureDef.shape = shape;
        fixtureDef.density = 0.0f;
        fixtureDef.restitution = 0.0f;
        fixtureDef.isSensor = true;

        Fixture f = body.createFixture(fixtureDef);
        f.setUserData("switch");

        this.interactionTarget = interactionTarget;

        this.callback = () -> {
            if (interactionTarget != null) {
                interactionTarget.interact();
                isActive = !isActive; // Toggle active state
            }
        };

        this.position = position;
        body.setTransform(position.x, position.y, 0);
    }

    @Override
    public void act(float deltaTime) {
    }

    @Override
    public void draw(Batch batch, float parentOpacity) {
        TextureRegion currentFrame = textures.getKeyFrame(isActive ? 1 : 0, true);
        batch.draw(currentFrame,
            body.getPosition().x - currentFrame.getRegionWidth() * Constants.UNIT_SCALE / 2,
            body.getPosition().y - 4 * Constants.UNIT_SCALE,
            currentFrame.getRegionWidth() * Constants.UNIT_SCALE,
            currentFrame.getRegionHeight() * Constants.UNIT_SCALE);
    }
}

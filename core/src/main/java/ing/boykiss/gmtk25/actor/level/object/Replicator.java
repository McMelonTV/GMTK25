...
public class Replicator extends Interactable {
    ...
    public Replicator(Vector2 position, String label, InteractionTarget target) {
        super(position, label, target);
    }

    public Replicator(Vector2 position, String label, InteractionTarget target, boolean isActive) {
        this(position, label, target);
        this.initialState = new Nullean(false, isActive);
        this.isActive = isActive;
    }

    public Replicator(Vector2 position, String label, InteractionTarget target, Supplier<Boolean> stateGetter) {
        this(position, label, target);
